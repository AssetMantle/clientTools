...
class Make extends AnyFunSuite {
  ...
  val immutableMetaProperties: Seq[MetaProperty] = Seq(
    MetaProperty(PropertyID(StringID("OrderIM1"), constants.Data.AccAddressDataTypeID), AccAddressData(testConstants.setup.balanceAccount.address).toAnyData),
    MetaProperty(PropertyID(StringID("OrderIM2"), constants.Data.BooleanDataTypeID), BooleanData(true).toAnyData),
    MetaProperty(PropertyID(StringID("OrderIM3"), constants.Data.DecDataTypeID), DecData(AttoNumber(utilities.Random.getRandomDouble)).toAnyData),
    MetaProperty(PropertyID(StringID("OrderIM4"), constants.Data.HeightDataTypeID), HeightData(Height(utilities.Random.getRandomAbsLong)).toAnyData),
    MetaProperty(PropertyID(StringID("OrderIM5"), constants.Data.ListDataTypeID), testConstants.Data.listData.toAnyData),
    MetaProperty(PropertyID(StringID("OrderIM6"), constants.Data.NumberDataTypeID), NumberData(utilities.Random.getRandomAbsLong).toAnyData),
    MetaProperty(PropertyID(StringID("OrderIM7"), constants.Data.StringDataTypeID), StringData(utilities.Random.getRandomString).toAnyData),
    MetaProperty(PropertyID(StringID("bondAmount"), constants.Data.NumberDataTypeID), NumberData(5916).toAnyData),
  )
  val immutableProperties: Seq[MesaProperty] = Seq(
    ...
    MesaProperty(PropertyID(StringID("OrderI3"), constants.Data.DecDataTypeID), DecData(AttoNumber(utilities.Random.getRandomDouble))),
  ...
  val mutableMetaProperties: Seq[MetaProperty] = Seq(
    MetaProperty(PropertyID(StringID("OrderMM1"), constants.Data.AccAddressDataTypeID), AccAddressData(utilities.Wallet.getRandomWallet.address).toAnyData),
    MetaProperty(PropertyID(StringID("OrderMM2"), constants.Data.BooleanDataTypeID), BooleanData(true).toAnyData),
    MetaProperty(PropertyID(StringID("OrderMM3"), constants.Data.DecDataTypeID), DecData(AttoNumber(utilities.Random.getRandomDouble)).toAnyData),
    MetaProperty(PropertyID(StringID("OrderMM4"), constants.Data.HeightDataTypeID), HeightData(Height(utilities.Random.getRandomAbsLong)).toAnyData),
    MetaProperty(PropertyID(StringID("OrderMM5"), constants.Data.ListDataTypeID), testConstants.Data.listData.toAnyData),
    MetaProperty(PropertyID(StringID("OrderMM6"), constants.Data.NumberDataTypeID), NumberData(utilities.Random.getRandomAbsLong).toAnyData),
    MetaProperty(PropertyID(StringID("OrderMM7"), constants.Data.StringDataTypeID), StringData(utilities.Random.getRandomString).toAnyData),
  )
  val mutableProperties: Seq[MesaProperty] = Seq(
    ...
    MesaProperty(PropertyID(StringID("OrderM3"), constants.Data.DecDataTypeID), DecData(AttoNumber(utilities.Random.getRandomDouble))),
  ...
  val makeMsg = utilities.BlockchainTransaction.getMakeOrderMsgAsAny(
    ...
    makerOwnableSplit = fullFormat.format(BigDecimal(1) / AttoNumber.factor),
