...
abstract class GenericDaoImpl3[
  ...
  def create(entity: E): Future[Unit] = db.run((tableQuery += entity).asTry).map {
    ...
    case Failure(exception) => exception match {
      case psqlException: PSQLException => throw new BaseException(new response.Failure(module + "_INSERT_FAILED"), psqlException)
    }
  }
  ...
  def customUpdate[R](updateQuery: DBIOAction[R, NoStream, Effect.Write]): Future[R] = db.run(updateQuery.asTry).map {
    case Success(result) => result match {
      case 0 => throw new BaseException(new response.Failure(module + "_NOT_FOUND"))
      case _ => result
    }
    case Failure(exception) => exception match {
      case psqlException: PSQLException => throw new BaseException(new response.Failure(module + "_INSERT_FAILED"), psqlException)
  ...
  def delete(id1: PK1, id2: PK2, id3: PK3): Future[Int] = db.run(tableQuery.filter(x => x.id1 === id1 && x.id2 === id2 && x.id3 === id3).delete.asTry).map {
    ...
    case Failure(exception) => exception match {
      case psqlException: PSQLException => throw new BaseException(new response.Failure(module + "_DELETE_FAILED"), psqlException)
      case noSuchElementException: NoSuchElementException => throw new BaseException(new response.Failure(module + "_DELETE_FAILED"), noSuchElementException)
    }
  }
  ...
  def deleteAll(): Future[Unit] = db.run(sqlu"""TRUNCATE TABLE ${tableQuery.baseTableRow.tableName} RESTART IDENTITY CASCADE""".asTry).map {
    case Success(result) => ()
    case Failure(exception) => exception match {
      case psqlException: PSQLException => throw new BaseException(new response.Failure(module + "_DELETE_ALL_FAILED"), psqlException)
    }
  }
  ...
  def filterHead[C <: Rep[_]](expr: T => C)(implicit wt: CanBeQueryCondition[C]): Future[E] = db.run(tableQuery.filter(expr).result.head.asTry).map {
    case Success(result) => result
    case Failure(exception) => exception match {
      case noSuchElementException: NoSuchElementException => throw new BaseException(new response.Failure(module + "_NOT_FOUND"), noSuchElementException)
    }
  }
  ...
  def updateById1Id2Id3(update: E): Future[Unit] = db.run(tableQuery.filter(x => x.id1 === update.id1 && x.id2 === update.id2 && x.id3 === update.id3).update(update).asTry).map {
    case Success(result) => ()
    case Failure(exception) => exception match {
      case psqlException: PSQLException => throw new BaseException(new response.Failure(module + "_UPDATE_FAILED"), psqlException)
      case noSuchElementException: NoSuchElementException => throw new BaseException(new response.Failure(module + "_UPDATE_FAILED"), noSuchElementException)
    }
  }
  ...
  def upsert(entity: E): Future[Unit] = db.run(tableQuery.insertOrUpdate(entity).asTry).map {
    case Success(result) => ()
    case Failure(exception) => exception match {
      case psqlException: PSQLException => throw new BaseException(new response.Failure(module + "_UPSERT_FAILED"), psqlException)
    }
  }
