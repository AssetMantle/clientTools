...
abstract class SuperData {

  def getType: StringID

  def getID: DataID

  def zeroValue: SuperData
...
object SuperData {
  ...
  def apply(anyData: AnyDataV1.AnyData): SuperData = anyData.getImplCase.getNumber match {
  ...
  def apply(protoBytes: Array[Byte]): SuperData = SuperData(AnyDataV1.AnyData.parseFrom(protoBytes))
